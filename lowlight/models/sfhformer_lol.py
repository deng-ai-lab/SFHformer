...
def GridPartition(mode, size=8):
    B, C, H, W = mode.shape
    mode = mode.view(B, C, H // size, size, W // size, size)  # B*3*H/2*2*W/2*2
    mode = mode.permute(0, 1, 3, 2, 5, 4).contiguous().view(B, C, H, W)
    return mode


def UnGridPartition(mode, size=8):
    B, C, H, W = mode.shape
    mode = mode.view(B, C, size, H // size, size, W // size)  # B*3*H/2*2*W/2*2
    mode = mode.permute(0, 1, 3, 2, 5, 4).contiguous().view(B, C, H, W)
    return mode
...
class OurTokenMixer_For_Local(nn.Module):
    def __init__(
        ...
        self.conv_init = nn.Sequential(  # PW->DW->
            nn.Conv2d(dim, dim*scale_ratio, 1),
            nn.GELU()
        )
        self.conv_fina = nn.Sequential(
            nn.Conv2d(dim*scale_ratio, dim, 1),
            nn.GELU()
        )
...
class FourierUnit(nn.Module):
    ...
    def forward(self, x):
        ...
        ffted = self.conv_layer1(self.bn1(ffted))  # (batch, c*2, h, w/2+1)
...
class OurMixer(nn.Module):
    def __init__(
        ...
        self.dim = dim
        self.mixer_local = token_mixer_for_local(dim=self.dim, kernel_size=mixer_kernel_size,
...
class Backbone_new(nn.Module):
    def __init__(self, in_chans=3, out_chans=3, patch_size=1,
        ...
        self.layer4 = OurStage(depth=depth[3], in_channels=embed_dim[3],
        ...
        self.upsample2 = PatchUnEmbed_for_upsample(patch_size=2, embed_dim=embed_dim[3],
                                                   out_dim=embed_dim[4])
        self.layer5 = OurStage(depth=depth[4], in_channels=embed_dim[4],
